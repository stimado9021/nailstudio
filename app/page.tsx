...
// ── Icons ─────────────────────────────────────────────────────────────────────
const Icon = ({ d, size = 20, color = "currentColor", stroke = false }: IconProps) => (
...
// ── Campañas iniciales demo ───────────────────────────────────────────────────
const initialCampaigns: Campaign[] = [
  { id: 1, name: "Lanzamiento Q1", subject: "¡Nuevo producto disponible!", status: "sent", sent: 1240, opens: 389, clicks: 142, date: "2025-01-15", brevoId: null },
  { id: 2, name: "Newsletter Febrero", subject: "Novedades de Febrero 🚀", status: "sent", sent: 3200, opens: 960, clicks: 287, date: "2025-02-01", brevoId: null },
];
...
const Input = ({ label, ...p }: InputProps) => (
...
const Btn = ({ children, variant = "primary", onClick, small, icon, disabled }: BtnProps) => {
...
// SETTINGS
// ════════════════════════════════════════════════════════════════════════════
function Settings({ config, setConfig }: { config: Config; setConfig: (c: Config) => void }) {
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      setTestResult(data.success ? { ok: true, account: data.account } : { ok: false, error: data.error });
    } catch {
  ...
  const field = (label: string, key: keyof Config, type = "text", ph = "") => (
    <div style={{ marginBottom: 16 }}>
      <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#444", marginBottom: 6 }}>{label}</label>
      <input type={type} value={config[key]} onChange={e => setConfig({ ...config, [key]: e.target.value })} placeholder={ph}
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #e8e4dc", borderRadius: 12, padding: 28 }}>
          ...
          <div style={{ background: "#fffbeb", border: "1px solid #fef08a", borderRadius: 8, padding: 12, marginBottom: 16 }}>
            <p style={{ margin: 0, fontSize: 12, color: "#854d0e" }}>
              ⚠️ La API Key real va en <strong>Vercel → Environment Variables → BREVO_API_KEY</strong>. Este campo es solo referencia visual.
            </p>
          </div>
          {field("API Key (referencia visual)", "apiKey", "password", "xkeysib-...")}
        ...
        <div style={{ background: "#fff", border: "1px solid #e8e4dc", borderRadius: 12, padding: 28 }}>
          ...
          {field("Reply-To", "replyTo", "email", "respuestas@miempresa.com")}
          <div style={{ background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: 8, padding: 12 }}>
            <p style={{ margin: 0, fontSize: 12, color: "#0369a1" }}>
              💡 El email del remitente debe estar <strong>verificado en Brevo</strong> (Senders &amp; IP → Senders).
            </p>
          </div>
        </div>
        ...
        <div style={{ background: "#fafaf8", border: "1px solid #e8e4dc", borderRadius: 12, padding: 28, gridColumn: "1 / -1" }}>
          <h3 style={{ fontFamily: "Georgia, serif", fontSize: 17, margin: "0 0 12px" }}>🚀 Variables de entorno en Vercel</h3>
          <p style={{ fontSize: 14, color: "#555", marginBottom: 12 }}>
            En Vercel → tu proyecto → <strong>Settings → Environment Variables</strong> agrega:
          </p>
          <pre style={{ background: "#1a1a1a", color: "#a8ff78", padding: 20, borderRadius: 10, fontSize: 14, overflowX: "auto", margin: "0 0 12px" }}>
{`BREVO_API_KEY = xkeysib-xxxxxxxxxxxxxxxxxxxxxxxxxx`}
          </pre>
          <p style={{ fontSize: 13, color: "#888", margin: 0 }}>
            Después de agregar la variable, haz <strong>Redeploy</strong> en Vercel para que tome efecto.
          </p>
        </div>
      </div>
...
// CAMPAIGNS
// ════════════════════════════════════════════════════════════════════════════
function Campaigns({ campaigns, setCampaigns, config }: { campaigns: Campaign[]; setCampaigns: React.Dispatch<React.SetStateAction<Campaign[]>>; config: Config }) {
  ...
  const [newCampaign, setNewCampaign] = useState({ name: "", subject: "", preheader: "", listId: "", templateId: "1" });
  ...
  const handleSend = async (camp: Campaign) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showToast(`✅ "${camp.name}" enviada correctamente (ID Brevo: ${data.campaignId})`);
      } else {
            ...
            <div key={c.id} style={{ background: "#fff", border: "1px solid #e8e4dc", borderRadius: 12, padding: 24, display: "flex", alignItems: "center", gap: 20 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ margin: 0, fontSize: 12, color: "#aaa" }}>
                  Lista ID: {c.listId ?? "—"} · Plantilla: {template?.name} · {c.date}
                </p>
              </div>
              {c.status === "sent" && (
                <div style={{ display: "flex", gap: 24, textAlign: "center" }}>
                  {([["Enviados", String(c.sent || "—")], ["Aperturas", c.sent ? `${((c.opens / c.sent) * 100).toFixed(0)}%` : "—"], ["Clics", c.sent ? `${((c.clicks / c.sent) * 100).toFixed(0)}%` : "—"]] as [string, string][]).map(([l, v]) => (
                    <div key={l}>
      ...
      {showNew && (
        <Modal title="Nueva campaña" onClose={() => setShowNew(false)} wide>
          <Input label="Nombre de la campaña (interno)" value={newCampaign.name}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewCampaign(p => ({ ...p, name: e.target.value }))} placeholder="Ej: Newsletter Mayo 2025" />
          <Input label="Asunto del email" value={newCampaign.subject}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewCampaign(p => ({ ...p, subject: e.target.value }))} placeholder="¡Novedad exclusiva para ti! 🎁" />
          <Input label="Texto de previsualización" value={newCampaign.preheader}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewCampaign(p => ({ ...p, preheader: e.target.value }))} placeholder="Texto que se ve antes de abrir..." />

          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#444", marginBottom: 6 }}>
              Lista de Brevo {loadingLists && <span style={{ color: "#aaa", fontWeight: 400 }}>— cargando...</span>}
            </label>
            {brevoLists.length > 0 ? (
              <select value={newCampaign.listId} onChange={e => setNewCampaign(p => ({ ...p, listId: e.target.value }))}
                ...
                {brevoLists.map(l => <option key={l.id} value={String(l.id)}>{l.name} ({l.uniqueSubscribers} suscriptores) — ID: {l.id}</option>)}
              </select>
            ) : (
              <input value={newCampaign.listId} onChange={e => setNewCampaign(p => ({ ...p, listId: e.target.value }))}
                placeholder="Ej: 2  (Brevo → Contacts → Lists → número)"
                style={{ width: "100%", padding: "10px 14px", border: "1.5px solid #e8e4dc", borderRadius: 8, fontSize: 14, outline: "none", boxSizing: "border-box", fontFamily: "inherit" }} />
            )}
          </div>
          ...
          <div style={{ background: "#fffbeb", border: "1px solid #fef08a", borderRadius: 8, padding: 12, marginBottom: 16 }}>
            <p style={{ margin: 0, fontSize: 13, color: "#854d0e" }}>
              ⚠️ Remitente configurado: <strong>{config.senderEmail || "(no configurado — ve a Configuración)"}</strong>
            </p>
          </div>

          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
            <Btn variant="ghost" onClick={() => setShowNew(false)}>Cancelar</Btn>
            <Btn icon="plus" onClick={handleCreate} disabled={!newCampaign.name || !newCampaign.subject}>
              Crear campaña
            </Btn>
          </div>
        </Modal>
...
// DASHBOARD
// ════════════════════════════════════════════════════════════════════════════
function Dashboard({ campaigns, config }: { campaigns: Campaign[]; config: Config }) {
  ...
  return (
    <div>
      ...
      {!config.senderEmail && (
        <div style={{ background: "#fffbeb", border: "1px solid #fef08a", borderRadius: 10, padding: "14px 20px", marginBottom: 24 }}>
          <p style={{ margin: 0, color: "#854d0e", fontSize: 14 }}>
            <strong>Configura tu cuenta</strong> — Ve a Configuración, agrega tu email remitente y prueba la conexión con Brevo.
          </p>
        </div>
      )}
      ...
      <div style={{ background: "#fff", border: "1px solid #e8e4dc", borderRadius: 12, padding: 24 }}>
        <h3 style={{ fontFamily: "Georgia, serif", fontSize: 18, margin: "0 0 20px" }}>Campañas recientes</h3>
        {campaigns.length === 0
          ? <p style={{ color: "#aaa", textAlign: "center", padding: 32 }}>Sin campañas aún.</p>
          : campaigns.map(c => (
            <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: "1px solid #f8f6f2" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                {c.brevoId && <span style={{ fontSize: 11, color: "#aaa", fontFamily: "monospace" }}>ID: {c.brevoId}</span>}
                {statusBadge(c.status)}
              </div>
            </div>
          ))
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: "#888", margin: 0 }}>Diseños HTML compatibles con Brevo</p>
      </div>
          ...
          <div key={t.id} style={{ background: "#fff", border: "1px solid #e8e4dc", borderRadius: 12, overflow: "hidden" }}>
            ...
            <div style={{ padding: 20 }}>
              ...
              <div style={{ background: "#f8f6f2", borderRadius: 8, padding: 10, marginBottom: 12 }}>
                <p style={{ margin: 0, fontSize: 11, color: "#888", fontFamily: "monospace" }}>VARIABLES</p>
                <p style={{ margin: "4px 0 0", fontSize: 11, color: "#555", fontFamily: "monospace" }}>{"{{contact.FIRSTNAME}}"} {"{{unsubscribe}}"}</p>
              </div>
...
export default function App() {
  ...
  const [config, setConfig] = useState<Config>({ senderName: "", senderEmail: "", replyTo: "", apiKey: "" });
