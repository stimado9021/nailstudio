...
export default function Campaigns({
  ...
  // ── Abrir modal y cargar listas de Brevo ────────────────────────────────────
  const openNew = async () => {
    setShowNew(true);
    ...
    try {
      ...
      if (data.success && data.lists?.length) setBrevoLists(data.lists);
    } catch {}
  ...
  const handleCreate = () => {
    ...
    const listName =
      brevoLists.find((l) => String(l.id) === form.listId)?.name ?? `Lista #${form.listId}`;
    setCampaigns((p) => [
      ...p,
      {
        ...form,
  ...
  // ── Enviar campaña real vía Brevo API ───────────────────────────────────────
  const handleSend = async (camp: Campaign) => {
    if (!config.senderEmail || !config.senderName) {
      toast("⚠️ Configura el remitente en Configuración antes de enviar", false);
      return;
    }
    if (!camp.listId) {
      toast("⚠️ Esta campaña no tiene lista de Brevo asignada", false);
      return;
    }

    const tmpl =
      emailTemplates.find((t) => String(t.id) === String(camp.templateId)) ?? emailTemplates[0];
    ...
    try {
      ...
      if (data.success) {
        ...
        toast(`✅ Campaña "${camp.name}" enviada (ID Brevo: ${data.campaignId})`);
      } else {
  ...
  const currentTemplate =
    emailTemplates.find((t) => String(t.id) === form.templateId) ?? emailTemplates[0];
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", marginBottom: 28, background: "#fff", border: "1px solid #e8e4dc", borderRadius: 12, overflow: "hidden" }}>
        {[
          { n: "1", label: "Sube contactos a Brevo",  sub: "Contactos → Subir a Brevo" },
          { n: "2", label: "Crea la campaña",          sub: "Lista, asunto y plantilla" },
          { n: "3", label: "Envía vía Brevo",          sub: "Los emails llegan a tus contactos" },
        ].map((s, i) => (
          <div
            key={i}
            style={{
          ...
          >
            <div
              style={{
                ...
                background: "#f8f6f2", border: "2px solid #e8e4dc",
                display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
              }}
            >
              <span style={{ fontSize: 13, fontWeight: 700, color: "#aaa" }}>{s.n}</span>
            </div>
            <div>
      ...
      {/* Lista de campañas o estado vacío */}
      {campaigns.length === 0 ? (
        <div style={{ background: "#fff", border: "2px dashed #e8e4dc", borderRadius: 12, padding: 48, textAlign: "center" }}>
          ...
          <div style={{ marginTop: 16 }}>
            <Btn icon="plus" onClick={openNew}>Crear primera campaña</Btn>
          </div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {campaigns.map((c) => {
            const tmpl =
              emailTemplates.find((t) => String(t.id) === String(c.templateId)) ?? emailTemplates[0];
            return (
              <div
                key={c.id}
                style={{
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ margin: 0, fontSize: 12, color: "#aaa" }}>
                    Lista: {c.listName ?? `#${c.listId}`} · {tmpl.name} · {c.date}
                  </p>
                </div>

                {(c.status === "draft" || c.status === "error") && (
                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                    <Btn variant="ghost" small icon="eye" onClick={() => setShowPrev(tmpl)}>
                      Preview
                    </Btn>
                    <Btn small icon="send" onClick={() => handleSend(c)}>
                      {c.status === "error" ? "Reintentar" : "Enviar"}
                    </Btn>
                  </div>
                )}
                {c.status === "sending" && (
                  <span style={{ fontSize: 13, color: "#1e40af", fontWeight: 600, flexShrink: 0 }}>
                    ⏳ Enviando...
                  </span>
                )}
                {c.status === "sent" && (
                  <span style={{ fontSize: 13, color: "#166534", fontWeight: 600, flexShrink: 0 }}>
                    ✅ Enviada
                  </span>
                )}
              </div>
      ...
      {showNew && (
        <Modal title="Nueva campaña" onClose={() => setShowNew(false)} wide>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
            <Field label="Nombre de campaña (interno)">
              <input
                ...
                placeholder="Newsletter Mayo 2025"
                style={inputStyle}
              />
            </Field>
            <Field label="Asunto del email">
              <input
                ...
                placeholder="¡Novedad exclusiva para ti! 🎁"
                style={inputStyle}
              />
            </Field>
          </div>

          <Field label="Preheader" note="Texto que aparece antes de abrir el email en el cliente de correo">
            <input
              value={form.preheader}
              onChange={(e) => setForm((p) => ({ ...p, preheader: e.target.value }))}
              placeholder="Una breve descripción que acompaña al asunto..."
              style={inputStyle}
            />
          </Field>

          <Field
            label={`Lista de Brevo${loadingLists ? " — cargando..." : ""}`}
            note="Ve a Contactos → Subir a Brevo primero para obtener el ID de lista"
          >
            {brevoLists.length > 0 ? (
              <select
                value={form.listId}
              ...
              >
                ...
                {brevoLists.map((l) => (
                  <option key={l.id} value={String(l.id)}>
                    {l.name} · {l.uniqueSubscribers} suscriptores · ID: {l.id}
                  </option>
                ))}
              </select>
            ) : (
              <input
                value={form.listId}
                onChange={(e) => setForm((p) => ({ ...p, listId: e.target.value }))}
                placeholder="Ej: 3  (el ID que obtuviste al subir contactos)"
                style={inputStyle}
              />
            )}
          </Field>
          ...
          <div style={{ border: "1px solid #e8e4dc", borderRadius: 10, overflow: "hidden", marginBottom: 20, height: 150, position: "relative", background: "#f8f6f2" }}>
            <div
              style={{ transform: "scale(0.25)", transformOrigin: "top center", width: 600, position: "absolute", top: 0, left: "50%", marginLeft: -300 }}
          ...
          <div
            style={{
              background: config.senderEmail ? "#f0f9ff" : "#fffbeb",
              border: `1px solid ${config.senderEmail ? "#bae6fd" : "#fef08a"}`,
              borderRadius: 8,
              padding: 12,
              marginBottom: 20,
            }}
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
            ...
            <Btn icon="plus" onClick={handleCreate} disabled={!form.name || !form.subject}>
              Crear campaña
            </Btn>
          </div>
        </Modal>
