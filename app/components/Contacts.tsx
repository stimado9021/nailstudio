...
import { LocalContact, UploadResult } from "./types";
import { Btn, Field, Modal, Icon, icons, inputStyle } from "./ui";

declare const XLSX: any; // Cargado en layout.tsx via CDN
        ...
        try {
          ...
          const parsed = rows
            .map((row, i) => ({
              ...
              email:  get(row, "email", "correo", "e-mail", "mail", "correo electrónico"),
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: "#888", margin: 0 }}>
            {contacts.length} contactos · Importa Excel o CSV y súbelos a Brevo
          </p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <Btn variant="ghost" icon="upload" onClick={() => setShowImport(true)}>
            Importar Excel / CSV
          </Btn>
      ...
      {uploadResult && (
        <div style={{ background: "#dcfce7", border: "1px solid #bbf7d0", borderRadius: 10, padding: "16px 20px", marginBottom: 20 }}>
          ...
          <p style={{ margin: "6px 0 0", color: "#166534", fontSize: 13 }}>
            ...
            ← Usa este número al crear campañas
          </p>
        </div>
      )}
      ...
      <div style={{ background: "#fff", border: "1px solid #e8e4dc", borderRadius: 12, overflow: "hidden" }}>
          ...
          <thead>
            <tr style={{ background: "#fafaf8", borderBottom: "2px solid #f0ede6" }}>
              {["Nombre", "Email", "Lista", "Estado"].map((h) => (
                <th key={h} style={{ textAlign: "left", padding: "12px 16px", fontSize: 11, color: "#888", fontWeight: 700, letterSpacing: 1, fontFamily: "monospace" }}>
                  {h}
                </th>
              ))}
            </tr>
            ...
            {visible.map((c) => (
              <tr key={c.id} style={{ borderBottom: "1px solid #f8f6f2" }}>
                ...
                <td style={{ padding: "13px 16px" }}>
                  <span style={{ background: "#f0ede6", padding: "3px 10px", borderRadius: 20, fontSize: 12 }}>
                    {c.list}
                  </span>
                </td>
                <td style={{ padding: "13px 16px" }}>
        ...
        {visible.length === 0 && (
          <p style={{ textAlign: "center", padding: 40, color: "#aaa" }}>Sin contactos</p>
        )}
      </div>
        ...
        <Modal title="Importar contactos" onClose={resetImport} wide>
          ...
          <div style={{ display: "flex", gap: 0, marginBottom: 24, background: "#f8f6f2", borderRadius: 10, padding: 4 }}>
            {(["excel", "csv"] as const).map((m) => (
              <button
                key={m}
                onClick={() => { setImportMode(m); setCsvText(""); setParsedPreview([]); }}
                style={{
          ...
          {importMode === "excel" ? (
            <>
              {/* Zona drop Excel */}
              <div
                style={{ background: "#f0f9ff", border: "2px dashed #93c5fd", borderRadius: 12, padding: 36, textAlign: "center", marginBottom: 20, cursor: "pointer" }}
              ...
              <div style={{ background: "#fafaf8", border: "1px solid #e8e4dc", borderRadius: 10, padding: 16, marginBottom: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  {[
                    { col: "Nombre / Name",  examples: "nombre, name, firstname" },
                ...
                <p style={{ margin: "10px 0 0", fontSize: 12, color: "#aaa" }}>
                  Si no existe columna "lista", todos van a la lista "Importados"
                </p>
              </div>
            </>
          ) : (
                  ...
                  <thead>
                    <tr style={{ background: "#f0ede6" }}>
                      {["Nombre", "Email", "Lista"].map((h) => (
                        <th key={h} style={{ textAlign: "left", padding: "8px 12px", fontSize: 11, color: "#888", fontWeight: 700, fontFamily: "monospace" }}>
                          {h}
                        </th>
                      ))}
                    </tr>
                    ...
                    {parsedPreview.slice(0, 5).map((c, i) => (
                      <tr key={i} style={{ borderTop: "1px solid #e8e4dc" }}>
                        <td style={{ padding: "8px 12px", fontSize: 13 }}>
                          {c.name || <em style={{ color: "#ccc" }}>sin nombre</em>}
                        </td>
                        <td style={{ padding: "8px 12px", fontSize: 12, fontFamily: "monospace", color: "#555" }}>
                          {c.email}
                        </td>
                        <td style={{ padding: "8px 12px" }}>
                          <span style={{ background: "#f0ede6", padding: "2px 8px", borderRadius: 12, fontSize: 11 }}>
                            {c.list}
                          </span>
                        </td>
                      </tr>
        ...
        <Modal title="Subir contactos a Brevo" onClose={() => setShowUpload(false)}>
          <div style={{ background: "#fffbeb", border: "1px solid #fef08a", borderRadius: 8, padding: 14, marginBottom: 20 }}>
            <p style={{ margin: 0, fontSize: 13, color: "#854d0e" }}>
              Se creará una lista nueva en Brevo. Guarda el <strong>ID de lista</strong> — lo necesitas al crear campañas.
            </p>
          </div>
        ...
        <Modal title="Añadir contacto manualmente" onClose={() => setShowAdd(false)}>
          ...
          <Field label="Lista">
            <input
              ...
              onChange={(e) => setNewContact((p) => ({ ...p, list: e.target.value }))}
