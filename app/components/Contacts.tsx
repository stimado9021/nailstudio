...
import { Btn, Field, Modal, inputStyle } from "./ui";

declare const XLSX: any;

// ── Supabase helper — se crea solo en el cliente ───────────────────────────
...
export default function Contacts({
  ...
  const loadContacts = async () => {
    ...
    try {
      ...
      toast(`❌ Error cargando contactos: ${e?.message || "revisa .env.local"}`, false);
    }
    setLoading(false);
  };
  ...
  const handleFile = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (/\.(xlsx|xls|xlsm)$/i.test(file.name)) {
      try {
        ...
        setParsedPreview(parsed); setCsvText("");
        toast(`📊 ${parsed.length} contactos detectados en el Excel`);
      } catch { toast("❌ No se pudo leer el Excel.", false); }
    } else {
      const reader = new FileReader();
      reader.onload = ev => {
        const text = (ev.target?.result as string) || "";
        setCsvText(text); setParsedPreview(parseCSV(text));
      };
  ...
  const handleImportLocal = async () => {
    ...
    try {
      ...
      if (error) throw error;
      await loadContacts();
      setShowImport(false); setCsvText(""); setParsedPreview([]);
      toast(`✅ ${toAdd.length} contactos guardados. Ahora usa "Subir a Brevo" para enviarlos.`);
    } catch (e: any) {
  ...
  const handleUploadToBrevo = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setShowUpload(false); setListName("");
  ...
  const handleAddManual = async () => {
    ...
    } catch (e: any) { toast(`❌ Error: ${e?.message}`, false); }
  };
    ...
    <div>
        ...
        <div>
          ...
          <p style={{ color: "#888", margin: 0 }}>
            {loading
              ...
              : `${contacts.length} guardados · ${synced} en Brevo · ${local} pendientes`}
          </p>
        </div>
      ...
      {uploadResult && (
        <div style={{ background: "#dcfce7", border: "1px solid #bbf7d0", borderRadius: 10, padding: "16px 20px", marginBottom: 20 }}>
          ...
          <p style={{ margin: "6px 0 0", color: "#166534", fontSize: 13 }}>
            ...
            {" "}· Ve a <strong>Campañas</strong> para usar esta lista
          </p>
        </div>
      )}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} style={{ textAlign: "center", padding: 48, color: "#aaa" }}>
                {contacts.length === 0
                  ? "No hay contactos. Importa un Excel o añade uno manualmente."
              ...
              <tr key={String(c.id)} style={{ borderBottom: "1px solid #f8f6f2" }}>
                ...
                <td style={{ padding: "13px 16px", textAlign: "right" }}>
                  <button onClick={() => handleDelete(c.id)}
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#ddd", padding: "4px 8px", fontSize: 18 }}
      ...
      {showImport && (
        <Modal title="Importar contactos" onClose={resetImport} wide>
          <div style={{ display: "flex", marginBottom: 24, background: "#f8f6f2", borderRadius: 10, padding: 4 }}>
            ...
            <>
              ...
              <div style={{ background: "#fafaf8", border: "1px solid #e8e4dc", borderRadius: 10, padding: 16, marginBottom: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  {[{ col: "Nombre", ex: "nombre, name, firstname" }, { col: "Email", ex: "email, correo, e-mail" }, { col: "Lista", ex: "lista, list, grupo" }].map(({ col, ex }) => (
          ...
          {parsedPreview.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <div style={{ background: "#dcfce7", border: "1px solid #bbf7d0", borderRadius: 8, padding: "12px 16px", marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <p style={{ margin: 0, color: "#166534", fontWeight: 600, fontSize: 14 }}>✅ {parsedPreview.length} contactos válidos</p>
              ...
              <div style={{ background: "#fafaf8", borderRadius: 8, overflow: "hidden", border: "1px solid #e8e4dc" }}>
                ...
                {parsedPreview.length > 5 && <p style={{ textAlign: "center", padding: 8, color: "#aaa", fontSize: 12, margin: 0, borderTop: "1px solid #e8e4dc" }}>... y {parsedPreview.length - 5} más</p>}
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
            ...
            <Btn icon="check" onClick={handleImportLocal} disabled={parsedPreview.length === 0 && !csvText.trim()}>
              Guardar {parsedPreview.length > 0 ? `${parsedPreview.length} contactos` : ""} en Supabase
            </Btn>
          </div>
        </Modal>
      )}
        ...
        <Modal title="Subir contactos a Brevo" onClose={() => setShowUpload(false)}>
          <div style={{ background: "#fffbeb", border: "1px solid #fef08a", borderRadius: 8, padding: 14, marginBottom: 20 }}>
            <p style={{ margin: 0, fontSize: 13, color: "#854d0e" }}>
              Se creará una lista en Brevo y el estado en Supabase se actualizará a <strong>"En Brevo"</strong>.
            </p>
          </div>
